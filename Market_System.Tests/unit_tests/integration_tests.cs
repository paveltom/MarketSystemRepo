    ...
    public class integration_tests
    {
        ...
        public void get_purchase_history_from_store_success()
        {
            try
            {
                ...
                string shouldbe1 = "product " + item_dto_1.GetID() + " quantity: " + bought_item_1.GetQuantity();
                string shouldbe2 = "product " + item_dto_2.GetID() + " quantity: " + bought_item_2.GetQuantity();
                string got = ms.GetStorePurchaseHistory(session_id, store_dto.StoreID);
                Assert.IsTrue(got.Contains(shouldbe1) && got.Contains(shouldbe2));
