...
namespace Market_System.DomainLayer
{
    ...
    public sealed class MarketSystem
    {
        ...
        public void AddStorePurchasePolicy(string sessionID, string storeID, List<string> newPolicyProperties)
        {
            try
            {
                string user_id = get_userid_from_session_id(sessionID);
                storeFacade.AddStorePurchasePolicy(user_id, storeID, newPolicyProperties);
        ...
        public void AddProductPurchasePolicy(string SessionID, string productID, List<string> newPolicyProps)
        {
            try
            {
                string userID = userFacade.get_userID_from_session(SessionID);
                //TODO:: WHY WE DO NOT USE NEWPOLICYPROPERTY ?
                storeFacade.AddProductPurchasePolicy(userID, GetStoreIdFromProductID(productID), productID, newPolicyProps);
            }
            catch (Exception e)
            {
                throw e;
        ...
        public void AddNewAdmin(string sessionID, string Other_username)
        {
            try
            {
                ...
                employeeRepo.addNewAdmin(other_UserID);
        ...
        public void UpdateAuction(string session, string productID, double newPrice, string card_number, string month, string year, string holder, string ccv, string id)
        {
            try
            {
                ...
                storeFacade.GetOwnersOfTheStore(store.FounderID, storeID).ForEach(o => notificationFacade.AddNewMessage(o, "Market", msg));
                storeFacade.GetManagersOfTheStore(store.FounderID, storeID).ForEach(m => notificationFacade.AddNewMessage(m, "Market", msg));
            }
